...
module Mscs
  ...
  class << self
    ...
    def open(open_type, open_name, cluster_handle=nil)
    ...
    def enumerate(enumerationtype, myhandle, dwtype)
      chrs = (0.chr * 260)
      ...
      open_enum, cluster_enum = begin
        ...
        when 'Network' ;  [ClusterNetworkOpenEnum, ClusterNetworkEnum]
        when 'Node' ;     [ClusterNodeOpenEnum, ClusterNodeEnum]
        end
      end
    ...
    class Group
    def add(hCluster, res_grp)
      res_grp = utf8_to_utf16le(res_grp)
      CreateClusterGroup.call(hCluster,res_grp)
    end

    def remove(hCluster, res_grp)
      hGroup=Mscs.open('Group', res_grp, hCluster)
      DeleteClusterGroup.call(hGroup)    # this needs to be the handle of the group not the name. enum groups...
    end

    def query(hCluster, res_grp)
      hGroup=Mscs.open('Group', res_grp, hCluster)
      Mscs.enumerate('Group',hGroup, CLUSTER_GROUP_ENUM_CONTAINS)
    end  
    end
      
    def resource_add(hCluster, res_name, res_type, res_grp)
    ...
    def resource_remove(hCluster, res_name)
      hResource=Mscs.open('Resource', res_name, hCluster)
      DeleteClusterResource.call(hResource)
    end

    def resource_query(hCluster, res_name)
      hResource = Mscs.open('Resource', res_name, hCluster)
      # if hResource is a Fix >0 then run this, otherwise irb shits the bed.  we should probably fix all the other calls after opens
      Mscs.enumerate('Resource', hResource, CLUSTER_RESOURCE_ENUM_DEPENDS)
    end


    def resource_property_set (action,cluster,resource,hash_res={})
    ...
    def resource_property_query (hCluster,resource,hash_res={})
      output=[]
      size="0" * 260
      chrs = (0.chr * 1024)
      buffer1=utf8_to_utf16le(chrs)
    ...
    def resource_dependency_add (hCluster, res_name, dependendson)
    ...
    def resource_dependency_remove (hCluster, res_name, dependendson)
        hres_name=Mscs.open('Resource', res_name, hCluster)
        hdependendson=Mscs.open('Resource', res_name, hCluster)
